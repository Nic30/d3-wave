...
export function renderBitLine(parent, data, signalWidth, waveRowHeight,
 ...
 * */
export function renderWaveRow(signalType, data, graph, parent){
    ...
    var waveRowY = d3.scaleLinear()
                     .domain([0, 1])
                     .range([0, waveRowHeight]);
    data = filterData(data, graph.sizes.row.range)
    if (signalType.name === T_BIT){
        var signalWidth = 1
        return renderBitLine(parent, data, signalWidth,
                              waveRowHeight, waveRowYpadding,
                              waveRowX, waveRowY);
    } else {
    	return renderBitsLine(parent, data, signalType.width,
                               waveRowHeight, waveRowYpadding, 
                               waveRowX, waveRowY);
    }
